...
set search_path = public, pgmq, extensions
as $$
declare v_attempt public.sis_execution_attempts%rowtype; v_next_id uuid; v_next_ordinal smallint; v_next_msg bigint;
begin
  ...
  if p_next_stage is null then
    update public.sis_execution_runs set status='READY_TO_FINALIZE',current_stage='FINALIZE',updated_at=now() where id=v_attempt.run_id;
    return jsonb_build_object('status','READY_TO_FINALIZE');
  end if;
  ...
  if not public.reserve_durable_sis_v1_budget(v_next_id, p_next_units, p_next_unit_kind) then
    raise exception 'durable SIS fallback budget unavailable';
  end if;
...
create or replace function public.finalize_durable_sis_v1(
  p_run_id uuid, p_outcome text, p_signal jsonb, p_decision jsonb
) returns jsonb
language plpgsql security definer
set search_path = public, extensions
as $$
declare v_run public.sis_execution_runs%rowtype; v_existing public.sis_execution_finalizations%rowtype; v_signal_id uuid; v_decision_id uuid;
begin
  ...
  if found then return jsonb_build_object('outcome',v_existing.outcome,'signal_id',v_existing.signal_id,'decision_log_id',v_existing.decision_log_id,'duplicate',true); end if;
  ...
  if p_outcome in ('SIGNAL','WEAK_SIGNAL') then
    ...
    values(p_signal->>'title',p_signal->>'description',(p_signal->>'category')::public.signal_category,
      case when p_outcome='WEAK_SIGNAL' then 'WEAK' else 'DRAFT' end::public.signal_status,
      (p_signal->>'impact_factor')::smallint,(p_signal->>'actor_factor')::smallint,(p_signal->>'novelty_factor')::smallint,
      (p_signal->>'verifiability_factor')::smallint,(p_signal->>'strategic_factor')::smallint,
      (p_signal->>'authority_factor')::smallint,(p_signal->>'corroboration_factor')::smallint,
      (p_signal->>'specificity_factor')::smallint,(p_signal->>'category_confidence_factor')::smallint,7,
      (p_signal->>'signal_score')::smallint,(p_signal->>'confidence_score')::smallint,
      coalesce((p_signal->>'momentum_score')::smallint,0),p_outcome,'PENDING',array['AWAITING_QUALITY_REVIEW'],
      'quality-foundation-v1',array[v_run.observation_id],(p_decision->>'sis_novelty')::numeric,
      (p_decision->>'sis_importance')::numeric,(p_decision->>'sis_urgency')::numeric,
      (p_decision->>'sis_confidence')::numeric,(p_decision->>'sis_final')::numeric,
      (p_decision->>'sis_final')::numeric,coalesce(p_decision->'human_relevance','{}'::jsonb),
      (p_decision->>'anti_hype_score')::numeric,coalesce(p_decision->'anti_hype_flags','{}'::jsonb),
      p_decision->>'relevance_horizon','ACTIVE','durable-sis-v1',false,
      jsonb_build_object('durable_sis_run_id',p_run_id)) returning id into v_signal_id;
  elsif p_outcome <> 'DISCARD' then raise exception 'invalid outcome'; end if;
  ...
  values(v_signal_id,v_run.observation_id,p_outcome,p_decision->>'rejection_code',p_decision->>'rejection_reason',
    p_decision->>'engine_justification',(p_decision->>'sis_final')::numeric,(p_decision->>'sis_novelty')::numeric,
    (p_decision->>'sis_importance')::numeric,(p_decision->>'sis_urgency')::numeric,
    (p_decision->>'sis_confidence')::numeric,(p_decision->>'sis_final')::numeric,
    coalesce(p_decision->'human_relevance','{}'::jsonb),(p_decision->>'anti_hype_score')::numeric,
    coalesce(p_decision->'anti_hype_flags','{}'::jsonb),'durable-sis-v1') returning id into v_decision_id;
  update public.observations set processed=true,signal_id=v_signal_id,processing_error=null,
    qualification_result=p_outcome,rejection_code=p_decision->>'rejection_code',
    rejection_reason=p_decision->>'rejection_reason',engine_version='durable-sis-v1'
  ...
  values(p_run_id,v_run.observation_id,p_outcome,v_signal_id,v_decision_id);
  update public.sis_execution_runs set status='FINALIZED',updated_at=now() where id=p_run_id;
  return jsonb_build_object('outcome',p_outcome,'signal_id',v_signal_id,'decision_log_id',v_decision_id,'duplicate',false);
end
$$;
...
revoke all on function public.complete_durable_sis_v1_attempt(uuid,bigint,text,jsonb,jsonb,text,text,text,integer,text) from public, anon, authenticated;
revoke all on function public.finalize_durable_sis_v1(uuid,text,jsonb,jsonb) from public, anon, authenticated;
...
grant execute on function public.complete_durable_sis_v1_attempt(uuid,bigint,text,jsonb,jsonb,text,text,text,integer,text) to service_role;
grant execute on function public.finalize_durable_sis_v1(uuid,text,jsonb,jsonb) to service_role;
