...
| Rollback                                         | https://vercel.com/docs/deployments/rollback-production-deployment                       | `vercel rollback <deployment-url-or-id>` is Vercel's official rollback mechanism. Rollback changes live production traffic. Vercel's own documentation discloses that rollback capability itself depends on plan tier: Hobby-tier accounts may only roll back to the immediately-previous production deployment, while Pro/Enterprise accounts may roll back to any eligible prior deployment. Whether this specific team's plan tier supports rolling back to an arbitrary prior deployment (versus only the immediately-previous one) has not been independently confirmed. Performing a rollback requires its own separate, explicit owner authorization at the time it is exercised (Section 7) — it is not pre-authorized by this document. |
