...
| Rollback to previous production deployment (REST/SDK) | https://vercel.com/docs/rest-api/sdk/projects/point-production-traffic-to-a-previous-production-deployment-by-id | Confirmed real and officially implemented, not undocumented: Vercel's own open-source CLI repository (`github.com/vercel/vercel`, `packages/cli/src/commands/rollback/request-rollback.ts`) contains a function literally named `requestRollback` that calls this exact operation — a separate operation from the normal-release promote endpoint, requiring a specific prior deployment ID, changing production traffic, and requiring its own separate explicit owner authorization to exercise. **Precise version discrepancy, disclosed rather than glossed over:** the CLI source code inspected during this correction pass calls `POST /v9/projects/{projectId}/rollback/{deploymentId}` — not `/v1/...` as this task's own instruction specified. The exact SDK documentation page URL given in the task, and which endpoint version its examples show, could not be independently confirmed via live search during this pass; this row cites the endpoint version directly observed in Vercel's own published CLI source code, which is treated here as at least as authoritative as an SDK reference page. |
